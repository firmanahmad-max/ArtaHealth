<#
  Regresi nutrition-scan (single-model) — Fase 4 §10 / docs/gate-nutrition-launch.md
  ----------------------------------------------------------------------------------
  ...
  Prasyarat:
    - Access token user (dari DevTools -> Local Storage -> sb-<ref>-auth-token -> access_token).
  ...
  dengan foto labelnya. Target §10: mayoritas 'ok', angka sesuai, latensi wajar.
#>
param(
...
Write-Host "Menemukan $($imgs.Count) gambar di $Dir. Mulai regresi...`n"
...
foreach ($f in $imgs) {
  ...
  try {
    ...
    if ($code -eq 401) { Write-Host "`n401 Unauthorized di file #$i ($($f.Name)) — token kadaluarsa. Refresh `$token, lalu jalankan lagi." -ForegroundColor Yellow; break }
    $bodyErr = ""; try { $bodyErr = (New-Object IO.StreamReader($_.Exception.Response.GetResponseStream())).ReadToEnd() } catch {}
    $status = if ($bodyErr -match 'not_a_label') { "not_a_label" } elseif ($code -gt 0) { "err_$code" } else { "err_local" }
  }
  $secs = [math]::Round($sw.Elapsed.TotalSeconds, 1)

  $e = $r.extracted; $s = $r.sanity; $ps = $e.per_serving
  $rows += [pscustomobject]@{
    file         = $f.Name
    status       = $status
    detik        = $secs
...
Write-Host "`n===== RINGKASAN ($($rows.Count) diproses) =====" -ForegroundColor Cyan
...
Write-Host "`nCSV: $Out  — buka & bandingkan tiap baris dengan foto labelnya untuk menilai akurasi." -ForegroundColor Green
